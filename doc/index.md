...
## 假设前提
- 通过智能合约能够使用某种数字代币进行转账行为。数字代币可以在特定网关/中间人进行法币兑换。
- 本方案中的各角色，能够授权本平台进行限定金额范围内的数字代币代扣、转账操作。
- 代各角色代扣、转账发生的行为，均在某区块链中有精确、不可修改的记录。
- 对于本段假设前提中设计的某区块链/某数字代币，不在本方案讨论范围内。
___
...
### 自动撮合流量主与品牌主
流量主与品牌主可自由更新自己所需维护的信息。当某角色所对应的信息发生变化时，促成其建立与对应角色的分销协议。
1. 用户（以流量主X为例）**更改（创建或修改）信息**（如可接受的前置定金范围、按件分成佣金范围）
1. 向智能合约A发送所修改的信息（所属用户、完整的用户信息）
1. 智能合约A遍历所有有记录的对应角色（品牌主）
1. 智能合约A返回所有匹配的品牌主（*基于配对条件）
1. 流量主X以站内信的形式获取匹配度最高的X个品牌主(所属用户、公开的用户信息)
1. 流量主X选择希望建立分销协议的品牌主，**发送邀请**
1. 向智能合约B1，B2分别发送双方信息（形如：{流量主X invite 品牌主Y},{流量主X invite 品牌主Z}）
1. 品牌主以站内信的形式获得邀请（发起者信息）
1. 品牌主Y**接受**邀请,品牌主Z**拒绝**邀请
1. 向智能合约B1发送接受信息（{Y invite X}），向智能合约B2发送拒绝信息（{Z decline X}）或不做任何操作
1. 智能合约B1收到了双向的信息，因此生成分销协议草稿{X&Y deal, 具体信息};B2收到拒绝信息或超时仍未收到双向信息，告知用户被拒绝
1. 智能合约B1将分销协议草稿{X&Y deal, 具体信息}发送给智能合约C
1. 智能合约C根据分销协议中的前置定金，代品牌主Y**发送转账**数字货币给到流量主X，如操作成功，**生成分销协议正文**
1. 分销协议正文的生成，表示成功建立X与Y的分销协议

### 自动结算商品分成
流量主与品牌主建立了分销协议后，流量主分发分销协议对应的店铺链接，进行销售。用户U进行产品购买。
1. 用户U购买产品并进行付款（如微信支付），法币进入品牌主Y账户
1. 用户U填写物流信息或其他可选信息
1. 生成订单，告知品牌主Y
1. 品牌主Y发货，用户U确认收货，**交易完成**
1. 向智能合约D发送交易信息{品牌主: Y, 流量主: X, 消费者: U, 产品: product, 金额: 99RMB}
1. 智能合约D验证是否有基于X与Y的分销协议正文
1. 智能合约D代品牌主Y发送对应于单件产品分成的数字货币给流量主X，若扣款失败（*惩罚机制）
1. 完成单件商品分成，并作相应记录
